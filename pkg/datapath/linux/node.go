...
func (n *linuxNodeHandler) NodeNeighInsert(newNode nodeTypes.Node) error {
	n.mutex.Lock()
	defer n.mutex.Unlock()
	return n.insertNeighbor(&newNode, n.getApproriateIfaceName())
}
	...
	if n.neighNextHopRefCount.Delete(nextHopStr) {
		...
		if found {
			...
			if err := netlink.NeighDel(neigh); err != nil {
				scopedLog.WithError(err).Warn("Failed to remove neighbor entry")
				return err
			}

			scopedLog.Debug("Removed node neighbor")
